...
- При одиночном нажатии кнопки `Монитор` высылает отчет по всем показаниям.
...
Начиная с версии 1.0.06 привязать внешний датчик можно через процедуру "Find and bind". Для ввода `Монитора` в этот режим нужно 5 раз быстро нажать на кнопку, после чего монитор начинает мограть зеленым и светодиодами. После этого на датчике из [проекта](https://github.com/slacky1965/ts0201_tz3000_zed) нужно быстро нажать на кнопку два раза (версия прошивки датчика не ниже 1.0.09).
...
- Нажать быстро пять раз - запустить процедуру "Find and bind".
- Нажать быстро шесть раз - перезагрузить Монитор.
...
- 1.0.04
	...
	- Немного измена процедура `factory reset`.
- 1.0.05
	- Добавлена [полная перезагрузка (очистка) экрана](https://github.com/slacky1965/air_quality_monitor_zrd/issues/15) два раза в сутки. В 03:00 и в 15:00.
	- Откорректировано получение значения `LQI`, когда `Монитор` работает не напрямую через координатор, а через роутер.
	- Исправлен [вывод температуры](https://github.com/slacky1965/air_quality_monitor_zrd/issues/16) на экран (было 1 - 10.7°С = 10.7°С, 2 - 9.6°С = 9.67°С).
	- Добавлен значок `динамик включен` и `динамик выключен` (визуально отображает включен или выключен звук).
	- Добавлен принудительный `rejoin` после обновления `ОТА`.
- 1.0.06
	- Исправлен [вывод влажности](https://github.com/slacky1965/air_quality_monitor_zrd/issues/16#issuecomment-3903385260) на экран (было 1 - 100% = 100%, 2 - 83% = 830%).
	- Добалена возможность привязать внешний датчик температуры из [проекта](https://github.com/slacky1965/ts0201_tz3000_zed) через процедуру "Find and bind" (версия прошивки датчика не ниже 1.0.09).
	- Удален принудительный `rejoin` после обновления.
	- Добавлен `анонс` устройства при старте.
	- Изменена процедура опроса кнопки.
